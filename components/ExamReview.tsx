...
const ExamReview: React.FC<ExamReviewProps> = ({ exam }) => {
  ...
  const generatePDF = () => {
    ...
    // Chain PDF operations correctly
    const pdfInstance = html2pdf().set(opt).from(element).toPdf();

    pdfInstance.get('pdf').then((pdf: PdfDocument) => {
    ...
    // Call save() on the original chain
    pdfInstance.save();
  };

  const isFormValid = (): boolean => {
    return Boolean(teacherInfo.teacherName && teacherInfo.examDate);
  };
  ...
  return (
    <div className="w-full">
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-md">
          ...
          <div className="p-6">
            ...
            <button
              ...
              className="mt-6 rounded bg-blue-500 px-4 py-2 text-white transition-colors hover:bg-blue-600 disabled:cursor-not-allowed disabled:bg-gray-300"
            >
              Generate PDF
            </button>
          </div>
      ...
      {/* Hidden PDF Content */}
      <div className="">
        <div ref={printableRef} className="mx-auto max-w-4xl bg-white p-8">
          <div className="mb-8 text-center">
            <h1 className="mb-2 text-2xl font-bold">{exam?.name}</h1>
